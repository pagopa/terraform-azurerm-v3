...
resource "azurerm_federated_identity_credential" "identity_credentials" {
  ...
  subject             = each.value.subject == "environment" ? "repo:${each.value.org}/${each.value.repository}:${each.value.credentials_scope}:${each.value.subject}" : "repo:${each.value.org}/${each.value.repository}:${each.value.subject}"
}
