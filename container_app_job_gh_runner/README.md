...
| <a name="input_network"></a> [network](#input\_network) | Existing VNet information and subnet CIDR block to use (must be /23). Optionally specify the subnet name | <pre>object({<br>    vnet_resource_group_name = string<br>    vnet_name                = string<br>    subnet_name              = optional(string, null)<br>    subnet_cidr_block        = string<br>  })</pre> | n/a | yes |
