...
| <a name="input_enable_immutability_policy"></a> [enable\_immutability\_policy](#input\_enable\_immutability\_policy) | Enable immutability policy for stored elements | `bool` | `false` | no |
...
| <a name="input_immutability_policy"></a> [immutability\_policy](#input\_immutability\_policy) | Properties to setup the immutability policy | <pre>object({<br>    allow_protected_append_writes = boolean<br>    period_since_creation_in_days = number<br>  })</pre> | <pre>{<br>  "allow_protected_append_writes": false,<br>  "period_since_creation_in_days": 730<br>}</pre> | no |
