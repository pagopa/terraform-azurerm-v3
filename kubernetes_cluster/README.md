...
| <a name="input_custom_logs_alerts"></a> [custom\_logs\_alerts](#input\_custom\_logs\_alerts) | Map of name = criteria objects | <pre>map(object({<br/>    # Assuming each.value includes this attribute for Kusto Query Language (KQL)<br/>    query = string<br/>    # Severity of the alert. Possible values include: 0, 1, 2, 3, or 4.<br/>    severity = number<br/>    # Time window for which data needs to be fetched for query (must be greater than or equal to frequency). Values must be between 5 and 2880 (inclusive).<br/>    time_window = number<br/>    # Evaluation operation for rule - 'GreaterThan', GreaterThanOrEqual', 'LessThan', or 'LessThanOrEqual'.<br/>    operator = string<br/>    # Result or count threshold based on which rule should be triggered. Values must be between 0 and 10000 inclusive.<br/>    threshold = number<br/>    # Frequency (in minutes) at which rule condition should be evaluated. Values must be between 5 and 1440 (inclusive).<br/>    frequency = number<br/>    # Custom subject override for all email ids in Azure action group.<br/>    email_subject = string<br/>    # Custom payload to be sent for all webhook payloads in alerting action.<br/>    custom_webhook_payload = string<br/>  }))</pre> | `{}` | no |
