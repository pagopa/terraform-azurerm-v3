    ...
    [
      ...
      [
        for rule in nsg.inbound_rules :
        {
          ...
          source_address_prefix        = length(rule.source_address_prefixes) == 0 && (!alltrue([for p in rule.source_address_prefixes : (regex("[A-Za-z\\*]", p) == null) ])) ? (contains(rule.source_address_prefixes, "*") ? "*": rule.source_address_prefix) : rule.source_address_prefixes[0]
