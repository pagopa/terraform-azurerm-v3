    ...
    [
      ...
      [
        for rule in nsg.inbound_rules :
        {
          ...
          source_address_prefixes      = rule.source_address_prefixes == null ? data.azurerm_subnet.subnet["${rule.source_subnet_name}-${rule.source_subnet_vnet_name}"].address_prefixes : (alltrue([for p in rule.source_address_prefixes : (regex("[A-Za-z\\*]", p) == null) ])  ? rule.source_address_prefixes : null)
          #source_address_prefix        = rule.source_address_prefix != null ? rule.source_address_prefix : null
          source_address_prefix        = rule.source_address_prefixes != null && (!alltrue([for p in rule.source_address_prefixes : (regex("[A-Za-z\\*]", p) == null) ])) ? (contains(rule.source_address_prefixes, "*") ? "*": rule.source_address_prefix) : rule.source_address_prefixes[0]
