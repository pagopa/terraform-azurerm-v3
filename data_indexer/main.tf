...
module "internal_storage_account" {
  source = "github.com/pagopa/terraform-azurerm-v3.git//storage_account?ref=v7.61.0"
...
resource "azurerm_private_endpoint" "blob" {
  name                = format("%s-blob-endpoint", module.internal_storage_account[0].name)
  location            = var.location
  resource_group_name = var.resource_group_name
  subnet_id           = var.internal_storage.private_endpoint_subnet_id

  private_service_connection {
    name                           = format("%s-blob", module.internal_storage_account[0].name)
